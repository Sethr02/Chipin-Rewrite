...
namespace Chipin_Rewrite.Controllers
{
    [Route("[controller]")]
    [ApiController]
    public class CallPaySharedListController : Controller
    {
        private readonly ChipinDbContext _context;
        private readonly ILogger<CallPayController> _logger;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public CallPaySharedListController(IHttpContextAccessor httpContextAccessor, ChipinDbContext context, ILogger<CallPayController> logger)
        {
            _context = context;
            _logger = logger;
        ...
        public async Task<IActionResult> GetKey([FromForm] CallPayPaymentParams paymentParams)
        {
            ...
            using (HttpClient httpClient = new HttpClient())
            {
                ...
                if (_httpContextAccessor.HttpContext.Session.GetString("CartTotal").IsNullOrEmpty())
                {
                    amount = paymentParams.Amount.ToString();
                }
                else
                {
                    amount = _httpContextAccessor.HttpContext.Session.GetString("CartTotal");
                }

                HttpContext.Session.SetString("Test", paymentParams.Amount.ToString());
                ...
                var requestData = new FormUrlEncodedContent(new[]
                {
                    ...
                    new KeyValuePair<string, string>("success_url", $"https://chipinrewritewebappservice.azurewebsites.net/CallPaySuccess"),
